  ...
  for (const file of files) {
    ...
    for (const key of Object.keys(fm)) {
      ...
      for (const target of targets) {
        ...
        // For genealogy edges from parent-declaring fields (children, offspring, etc),
        // invert source/target so all genealogy edges go child→parent direction.
        let source = file.path;
        let targetPath = resolved.path;
        let edgeType = type.name;
        if (type.genealogy && isParentDeclaringField(key)) {
          [source, targetPath] = [targetPath, source];
          edgeType = 'parents';
        }
        ...
        notePaths.add(source);
        notePaths.add(targetPath);
      }
    }
...
/**
 * Detect if a frontmatter field represents a parent declaring a child
 * (as opposed to a child declaring a parent). Parent-declaring fields
 * need direction inversion so genealogy edges always go child→parent.
 */
function isParentDeclaringField(key: string): boolean {
  const normalized = key.toLowerCase();
  const parentDeclaring = ['children', 'offspring', 'kinder', 'nachkommen'];
  return parentDeclaring.includes(normalized);
}
