...
describe("buildFullGraph alias accumulation", () => {
	...
	describe("unresolved links", () => {
		...
		it("produces a phantom node/edge but no alias entry for an unresolved link", () => {
			const result = buildFullGraph(mockApp as App, settings, null);
			// Unresolved links render as phantom nodes rather than being dropped —
			// alias tracking only applies to links that resolved to a real note.
